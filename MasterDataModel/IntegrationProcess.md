...
This paper is intended to clarify the **virtual integration** process of the OIH framework. In contrast to a **materialized integration** (with a centralized data storage) the origin data will stay in the source database, but must by demand be transformed into the common data format (generic fields) of the integrated databases. A reliable integration must ensures a fluent interoperability between the associated data models and therewith a seamless business process. Therefore an application response time should not be [perceivable](https://stackoverflow.com/questions/536300/what-is-the-shortest-perceivable-application-response-delay) - due to delays.
...
## The schema
The master data schema consists of those data fields, that need to be transformed between data models within a custom integration environment (OIH instance). A master data model is derivable out of those generic data fields and its structure (tree) reflects the contextual usage (aka. bounded context) within repetitive business processes. Aggregates - data flow patterns - will help to design endpoints according to typical business processes (records) - of views (aka. interfaces).
